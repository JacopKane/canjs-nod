define([
	...
	nod.Control = can.Control({
		...
		respond						: function(status, deferred, args) {
			var deferredArguments, method, state;

			if(typeof status !== 'boolean') return false;
			if(!deferred) return false;

			deferredArguments	= this.arraySlice(arguments, 2);
		...
		render				: function() {
			...
			return this.deferMap(this.styles, function(name, key) {
			...
			.then(function() {
				return this.deferMap(this.templates, function(name, key) {
		...
		renderView			: function(name, viewOptions, data) {
			...
			this.require(viewOptions.type, 'view', 'can')
				...
				.then(function(templateName, idName, name) {
					...
					element = element[viewOptions.insertAs]
						(can.view(idName, data));
		...
		renderStyle			: function(name, styleOptions) {
			styleOptions = $.extend(true, this.options.render.style, styleOptions, {
				name : name });
		...
		loadCss				: function($styleLink, options) {
			var afterCss		= can.$.Deferred();

			if(!$styleLink || !$styleLink.length)
				return this.applyReject(afterCss, arguments);
			
			$styleLink.load(function(responseText, textStatus, XMLHttpRequest) {
		...
		loadLess			: function($styleLink, options) {
